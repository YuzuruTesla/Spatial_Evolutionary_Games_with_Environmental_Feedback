% This script generates Figure 4 (a), (b), and (c), illustrating the spatial patterns of populations u & v, 
% payoffs f_L & f_H, and the environmental metric n. Additionally, it calculates the average Riemann sum at t = 10^6. 
% To improve accuracy, both the left and right Riemann sums are computed, and their average is taken.
%
% The sensitivity of environmental motion for L-type individuals, chi_u, is set slightly above the critical value of chi_u=0.0647. 
% Under this chi_u, there exists only one wavenumber, m = 7.
...
%Plot f_L and f_H at t=max (Figure 4a)
figure;
...
xlabel('Position $x$', 'fontsize', 23, 'Interpreter', 'latex');
ylabel('Payoff', 'fontsize', 23, 'Interpreter', 'latex');  % General label for both \pi_L and \pi_H
ylim([0.39, 0.42]);
...
set(gca, 'FontSize', 16);
ax = gca;
ax.XLabel.FontSize = 23;  % Explicitly set X label size to 22
ax.YLabel.FontSize = 23;  % Explicitly set Y label size to 22
grid on;


% Plot u and v at t=max (Figure 4c)
figure;
