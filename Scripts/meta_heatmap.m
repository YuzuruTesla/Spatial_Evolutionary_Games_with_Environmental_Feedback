% This script generates Figure 8(b), which presents a heatmap showing how population u 
% merges into advantageous environments during the onset of oscillations, with corresponding impacts on payoffs.
% 
% This script, in combination with the script named meta.m, forms the complete Figure 8, illustrating the 
% time-dependent behavior of the patterned states.For the choice of environmental sensitivity, chi_u, 
% we set chi_u = 0.1, which is significantly above the critical threshold.
...
% Preallocate arrays for f_L and f_H
f_L = zeros(length(t), length(x));
f_H = zeros(length(t), length(x));
...
for i = 1:length(t)
    ...
    f_L(i, :) = (1 - n(i, :)) .* (R_0 .* frac + S_0 .* (1 - frac)) + n(i, :) .* (R_1 .* frac + S_1 .* (1 - frac));
    f_H(i, :) = (1 - n(i, :)) .* (T_0 .* frac + P_0 .* (1 - frac)) + n(i, :) .* (T_1 .* frac + P_1 .* (1 - frac));
end
