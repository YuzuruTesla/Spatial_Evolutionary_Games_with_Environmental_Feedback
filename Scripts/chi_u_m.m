...
% value of chi_u^*(m) occurring at m^* = 7 with \chi_u^* = 0.0646. 
...
m_values_a = 1:60; % Integers from 1 to 60
m_values_b = 3:10; % Integers from 3 to 10
chi_u_values_a = arrayfun(chi_u_star, m_values_a); % Evaluate the critical sensitivity for each m value
chi_u_values_b = arrayfun(chi_u_star, m_values_b);

%Figure1 (a)
figure;
scatter(m_values_a, chi_u_values_a, 60, 'filled'); 
...
%Figure1 (b)
figure;
scatter(m_values_b, chi_u_values_b, 90, 'filled'); 
xlabel('Wavenumber $m$', 'fontsize', 23, 'fontname', 'arial', 'Interpreter', 'latex'); 
ylabel('Critical Sensitivity $\chi_u^*(m)$', 'fontsize', 23, 'fontname', 'arial', 'Interpreter', 'latex');
set(gca, 'FontSize', 16); 
ax = gca;
